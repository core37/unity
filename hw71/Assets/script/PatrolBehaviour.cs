...
public class PatrolBehaviour : MonoBehaviour {
    ...
    void Start () {
        ...
        isCatching = false;
        if (ownIndex != 1) addAction.addRandomMovement(this.gameObject, false);
    }

	void Update () {
        checkNearByHero();
        count ++;
        if (count%100 == 0 && !isCatching)
    ...
    void OnCollisionStay(Collision e) {
        //撞击围栏，选择下一个点移动
        if (e.gameObject.name.Contains("Patrol") || e.gameObject.name.Contains("Bondary")
            || e.gameObject.name.Contains("Cross")) {
            isCatching = false;
            addAction.addRandomMovement(this.gameObject, false);
        }
