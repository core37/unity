...
public class GameModel : SSActionManager, ISSActionCallback {
    ...
    private const float PERSON_SPEED_NORMAL = 0.04f;
    private const float PERSON_SPEED_CATCHING = 0.03f;
    ...
    public void addRandomMovement(GameObject sourceObj, bool isActive) {
        ...
        float rr = (float)Random.Range(0, 101) / 100;
        Debug.Log(rr);
        switch (randomDir) {
            case Diretion.UP:
                target += new Vector3(0, 0, 3 + 1*rr );
                break;
            case Diretion.DOWN:
                target += new Vector3(0, 0, -(3 + 1*rr));
                break;
            case Diretion.LEFT:
                target += new Vector3(-(3 + 1*rr), 0, 0);
                break;
            case Diretion.RIGHT:
                target += new Vector3(3 + 1*rr, 0, 0);
                break;
